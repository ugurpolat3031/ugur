...
client.on('guildMemberAdd', (member, guild) => {
  ...
  require("moment-duration-format");
    ...
    ${ates} Kayıt olmak için solda bulunan **The Jes Forest** odalarına giriş yapıp ses teyit vermelisin.
  ...
  client.on('message', async message => {
    ...
    // Burdan altını birinden aldım ama kimden hatırlamıyom bilen dm gelsin
  // dc: Kardoxa#0126
/////////////// TAG ROL //////////////////
client.on("userUpdate", async function(oldUser, newUser) { 
    ...
    const chat = (ayarlar.sohbet)
    const taglog = (ayarlar.taglog)

    const guild = client.guilds.cache.get(guildID)
    const role = guild.roles.cache.find(roleInfo => roleInfo.id === roleID)
    const member = guild.members.cache.get(newUser.id)
    const embed = new Discord.MessageEmbed().setAuthor(member.displayName, member.user.avatarURL({ dynamic: true })).setColor('#ff0010').setTimestamp().setFooter(ayarlar.footer);
    if (newUser.username !== oldUser.username) {
        if (oldUser.username.includes(tag) && !newUser.username.includes(tag)) {
            ...
            client.channels.cache.get(chat).send(`**Harika! ${newUser} Tagımızı alarak ailemize katıldı! Bizde seni bekliyorduk <3**`)
